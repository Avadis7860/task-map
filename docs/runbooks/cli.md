...
`src/taskmap/cli.py:150` (main) · `:108` (build_parser) · main = entrypoint console.
...
`src/taskmap/cli.py:53` · appelé par tous les handlers.
...
`src/taskmap/cli.py:46` · appelé par chaque handler.
...
`src/taskmap/cli.py:65` / `:70` / `:75` · `set_defaults(func=…)`.
...
`src/taskmap/cli.py:80` / `:84` (délèguent) · `:88` (`_run_edit`, cœur partagé).
...
`src/taskmap/cli.py:33` · action argparse de `--schema-version`.
